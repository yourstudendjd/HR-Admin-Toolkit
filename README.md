...
一个基于 PyQt5 的桌面工具，持续迭代中。当前包含四个功能模块：
...
| 对账数据清洗 | 客户对账单去重分类，按客户型号+加工类型+加工要求组合 |
...
- **组合键**：客户型号 + 加工类型 + 加工要求（三者相同才合并）
- **规格**不参与去重组合（规格不同但前三项相同，仍合并为一条）
- **单价冲突策略**：取首次值 / 平均值 / 最大值 / 报错
- **自动处理**：合并单元格表头、单位说明行过滤
- **批量处理**：支持多文件同时添加，每文件独立模糊匹配列名，合并为一个工作簿（多工作表）
- **列名模糊匹配**：自动识别常见列名变体（如"客户产品型号"→"客户型号"、"单价（元/个）"→"单价"等）
